...
function tick() {
        ...
        if (
            ...
            for (const p of game.smartpr_queue) {
                ...
                if (p.update && p.mode <= 3) {
                    ...
                    switch (p.mode) {
                        case 0:
                            str += "" + format_lvl(p.goal) + "级"
                            break
                        case 1:
                            str += format_num(p.goal) + "放大倍率"
                            break
                        case 2:
                            str += format_num(p.goal) + "转生点"
                            break
                        case 3:
                            str += format_eff(p.goal) + "秒"
                            break
                    }
                }
                if (game.smartpr_phase === p.id) {
    ...
    if (
        ...
        let base_exp =
            "经验值基础产量：每秒" + format_infinity(eps) + "经验值"
        let effective_exp =
            "有效经验值产量：每秒" +
    ...
    if (game.notation === 8) {
        document.getElementById("version").innerHTML =
            "<br><br><br>经验模拟器 v?.?.?<br>作者 ???<br><br>最近更新时间： ???"
    } else {
        document.getElementById("version").innerHTML =
            "<br><br><br>经验模拟器 v2.15.4<br>作者 Zakuro<br><br>最近更新时间： March 26, 2025"
    }
}
